...
function criarCardSobre() {
    ...
    if (grid) {
        ...
        card.onclick = (e) => { e.preventDefault(); abrirModalInfo('sobre'); };
        card.innerHTML = `
...
async function carregarAtivos() {
    try {
        const response = await fetch('http://localhost:3000/api/ativos?usuario_id=' + usuarioId); 
        if (!response.ok) {
            const errObj = await response.json().catch(() => ({}));
            throw new Error(errObj.erro || 'Erro na API de Ativos (' + response.status + ')');
        }
        ...
        ativos.forEach((ativo, index) => {
            ...
            let classeBolinha = ativo.status === 'ativo' ? 'dot-ativo' : (ativo.status === 'manutencao' ? 'dot-manutencao' : 'dot-descartado');
            let textoStatus = ativo.status === 'ativo' ? 'Ativo' : (ativo.status === 'manutencao' ? 'Em Manutenção' : 'Inativo');
        ...
        console.error(error);
        const tbodyResumo = document.getElementById('corpo-tabela');
        if (tbodyResumo) tbodyResumo.innerHTML = `<tr><td colspan="5" style="color:red; text-align:center;">Falha: ${error.message === 'Failed to fetch' ? 'Servidor Node.js Offline ou Indisponível' : error.message}</td></tr>`;
    }
}
...
async function carregarLicencas() {
    try {
        const response = await fetch('http://localhost:3000/api/licencas?usuario_id=' + usuarioId); 
        ...
        licencas.forEach(lic => {
            const dataCompra = lic.data_compra ? new Date(lic.data_compra).toLocaleDateString('pt-BR', {timeZone: 'UTC'}) : 'N/A';
            const dataVenc = lic.data_expiracao ? new Date(lic.data_expiracao).toLocaleDateString('pt-BR', {timeZone: 'UTC'}) : 'N/A';

            if (lic.status === 'cancelada') {
                temCancelada = true;
                const dataCanc = lic.data_cancelamento ? new Date(lic.data_cancelamento).toLocaleDateString('pt-BR', {timeZone: 'UTC'}) : 'N/A';
        ...
        if (!temCancelada) {
            tbodyCanceladas.innerHTML = '<tr><td colspan="4" style="text-align: center; color: #a59caf;">Nenhuma licença cancelada até o momento.</td></tr>';
        }
...
document.getElementById('form-nova-licenca').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            carregarLicencas();
            mostrarAlertaCustom('Licença Registrada', 'O software e a licença foram vinculados ao sistema com sucesso!', 'success', '🔐');
        } else {
...
document.getElementById('form-renovar-licenca').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch(`http://localhost:3000/api/licencas/${id}/renovar`, { 
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (response.ok) {
            ...
            mostrarAlertaCustom('Renovação Concluída', `A licença do ${nomeSoftware} agora é válida até: ${dataBrasileira} 📅`, 'success', '⭐');
        } else {
...
document.getElementById('form-cancelar-licenca').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch(`http://localhost:3000/api/licencas/${id}/cancelar`, { 
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (response.ok) {
            fecharModalCancelar();
            carregarLicencas();
            mostrarAlertaCustom('Licença Cancelada', `A licença do ${nomeSoftware} foi movida para o histórico de cancelados.`, 'info', '🚫');
        } else {
...
document.getElementById('form-novo-pc').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (!querGerarAutomatico && codigoFinal === '') { 
            mostrarAlertaCustom('Atenção!', 'Por favor, digite o Código do Ativo.', 'info', '⚠️'); 
            if (inputCodigoLcl) {
        ...
        if (!nomeEl || !deptoEl || !statusEl) {
            throw new Error("Campos do formulário não encontrados no HTML (Verifique os IDs no ficheiro html).");
        }
        ...
        if (response.ok) {
            ...
            if (codigoInjetadoFoiGerado) {
                mostrarAlertaCustom('Computador Cadastrado!', `🤖 Código gerado: ${codigoFinal}\n\n⚠️ IMPORTANTE:\nSe este computador possuir uma etiqueta física, você pode editar depois.`, 'info', '💻');
            } else {
                mostrarAlertaCustom('Tudo Certo!', `Computador salvo com código: ${codigoFinal}`, 'success', '✅');
            }
        } else {
            const dataErro = await response.json();
            mostrarAlertaCustom('Atenção', dataErro.erro || 'Falha ao cadastrar o computador.', 'error', '❌');
        }
    } catch (error) {
        console.error(error);
        mostrarAlertaCustom('Falha de Sistema', error.message === 'Failed to fetch' ? 'Servidor Node.js Offline' : error.message, 'error', '❌');
    }
});
...
document.getElementById('form-excluir-pc').addEventListener('submit', async (e) => {
    ...
    try {
        const res = await fetch(`http://localhost:3000/api/ativos/${id}`, { method: 'DELETE' });
        if (res.ok) {
            fecharModalExcluir();
            carregarAtivos();
            mostrarAlertaCustom('Excluído', 'Removido com sucesso.', 'success', '🗑️');
        }
    } catch (error) {
        console.error(error);
    }
...
function filtrarAtivosPorStatus(status) {
    ...
    linhas.forEach(linha => {
        ...
        const textoStatus = linha.querySelectorAll('td')[4] ? linha.querySelectorAll('td')[4].textContent.trim() : '';
...
window.onclick = function(event) {
    if (!event.target.matches('.btn-opcoes')) {
        document.querySelectorAll('.menu-acoes-oculto.mostrar').forEach(menu => menu.classList.remove('mostrar'));
    }
